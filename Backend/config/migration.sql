...
-- =============================================================================
-- 9. hotels  (has its own SERIAL PK; place_id is FK to places)
-- =============================================================================
CREATE TABLE hotels (
  hotel_id              SERIAL PRIMARY KEY,
  -- One-to-one profile mapping between a hotel and its place row.
  place_id              INTEGER NOT NULL UNIQUE REFERENCES places (place_id) ON DELETE CASCADE,
  nearby_place_id       INTEGER REFERENCES places (place_id) ON DELETE SET NULL,
...
CREATE TABLE trip_plans (
  ...
  budget_currency      VARCHAR(10)        NOT NULL DEFAULT 'LKR',
  hotel_place_id       INTEGER            REFERENCES places (place_id) ON DELETE SET NULL,
  hotel_name           VARCHAR(200),
  hotel_category       VARCHAR(100),
  hotel_star_class     INTEGER            CHECK (hotel_star_class BETWEEN 0 AND 5),
  hotel_price_min      DECIMAL(12, 2)     NOT NULL DEFAULT 0,
  hotel_price_currency VARCHAR(10)        NOT NULL DEFAULT 'LKR',
  selected_places      JSONB              NOT NULL DEFAULT '[]',
  selected_hotels      JSONB              NOT NULL DEFAULT '[]',
...
CREATE TABLE reviews (
  ...
  status          enum_reviews_status NOT NULL DEFAULT 'approved',
  response        JSONB,
  "createdAt"     TIMESTAMPTZ  NOT NULL DEFAULT NOW(),
  "updatedAt"     TIMESTAMPTZ  NOT NULL DEFAULT NOW()
);
