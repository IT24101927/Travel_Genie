...
// Body: place fields (name, description, district_id, lat, lng, address_text) + hotel fields
exports.createHotel = async (req, res, next) => {
  try {
    const { hotel_type, price_per_night, star_class, check_in_time, check_out_time,
            ...
            name, description, district_id, lat, lng, address_text } = req.body;

    // 1. Create Place supertype
    const place = await Place.create({ name, description, district_id, lat, lng, address_text });
    ...
    const result = await Hotel.findByPk(hotel.hotel_id, { include: [placeInclude] });
