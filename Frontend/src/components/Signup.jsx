...
import { validateName, validatePhone, validateNic, validateEmail, validatePassword, validateDateOfBirth, normalizePhone as normPhone } from '../utils/validation'
...
function Signup({ theme, toggleTheme }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const pwCheck2 = validatePassword(formData.password)
    if (!pwCheck2.valid) {
      setError(pwCheck2.message)
      return
    }
    if (formData.nic && !isValidSriLankanNic(formData.nic)) {
    ...
    try {
      ...
      localStorage.setItem('currentUser', JSON.stringify({
        ...
        dateOfBirth: u.date_of_birth || formData.dateOfBirth,
      ...
      if (/\bid already exists\b/i.test(message)) {
        setError('Registration failed due to a temporary database issue. Please try again.')
      } else {
        setError(message)
      }
    } finally {
  ...
  return (
    <div className="sg-page">
      ...
      <div className="sg-right">
        <Link to="/" className="sg-back-home" aria-label="Back to Home">
