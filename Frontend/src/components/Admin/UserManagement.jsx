...
function UserManagement({ theme, toggleTheme }) {
  ...
  const travelStyleOptions = ['Adventure', 'Relax', 'Culture', 'Luxury']
  ...
  const handleAdd = () => {
    setModalMode('add')
    setFormData({ name: '', email: '', password: '', phone: '', role: 'user', isActive: true, interests: [], travelStyle: 'Culture' })
    setShowModal(true)
  }
  ...
  const handleEdit = (user) => {
    ...
    setFormData({
      ...
      travelStyle: user.preferences?.travelStyle || 'Culture'
    })
    setShowModal(true)
  }
    ...
    try {
      if (modalMode === 'add') {
        const body = {
          ...
          password: formData.password || 'TravelGenie@123',
          phone: formData.phone, role: formData.role, isActive: formData.isActive,
          interests: formData.interests,
          preferences: { travelStyle: formData.travelStyle }
        }
        const res = await fetch(`${API}/users/register`, { method: 'POST', headers: authH(), body: JSON.stringify(body) })
        const data = await res.json()
        if (data.success) { showToast('✅ User created successfully'); await loadUsers() }
        else setError(data.message || 'Failed to create user')
      } else {
        const body = {
          name: formData.name, phone: formData.phone, role: formData.role,
          isActive: formData.isActive, interests: formData.interests,
          preferences: { travelStyle: formData.travelStyle }
        }
  ...
  const filteredUsers = users
    .filter(user => {
      ...
      const matchesTravelStyle = filterTravelStyle === 'all' || user.preferences?.travelStyle === filterTravelStyle
  ...
  return (
    <div className="user-management">
      ...
      {/* Filters */}
      <div className="um-filters glass-card">
                ...
                <tr key={user.id}>
                  ...
                  <td>
                    {user.preferences?.travelStyle ? (
                      <span className={`travel-style-badge ${user.preferences.travelStyle.toLowerCase()}`}>
                        {user.preferences.travelStyle === 'Adventure' ? '🏔️' : 
                         user.preferences.travelStyle === 'Relax' ? '🏖️' : 
                         user.preferences.travelStyle === 'Culture' ? '🏛️' : '💎'} 
                        {user.preferences.travelStyle}
                      </span>
                    ) : <span>—</span>}
                  </td>
            ...
            <div className="user-details">
              ...
              <div className="details-grid">
                ...
                <div className="detail-group">
                  ...
                  <p>{selectedUser.preferences?.travelStyle || '—'}</p>
                </div>
