...
/* ── Single review card ─────────────────────────────────────── */
function ReviewCard({ review, currentUserId, onDelete, onEdit, onHelpful }) {
...
function ReviewForm({ targetType, targetName, onSubmit, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    await onSubmit({ rating, title: title.trim(), comment: comment.trim(), travelType, visit_date: visitDate || null, wouldRecommend: recommend, pros, cons })
    setSubmitting(false)
  }
...
export default function ReviewSection({ targetType, targetId, targetName, districtName = '', dbId }) {
  ...
  const handleDelete = async (review) => {
    ...
    if (review.id && !String(review.id).startsWith('local_')) {
      const token = getToken()
      try {
        await fetch(`${API}/reviews/${review.review_id || review.id}`, {
          method: 'DELETE',
