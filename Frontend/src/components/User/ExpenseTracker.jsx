...
export default function ExpenseTracker({ theme, toggleTheme }) {
  ...
  useEffect(() => {
    ...
    const applyTrips = (tripList) => {
      ...
      t.forEach(trip => {
        ...
        if (stored[tid] === undefined && trip.totalBudget > 0) {
          stored[tid] = trip.totalBudget
          changed = true
        }
      })
      if (changed) {
      ...
      fetch(`${API}/trips/my`, { headers: { Authorization: `Bearer ${token}` } })
        ...
        .then(json => {
          if (json?.success && Array.isArray(json.data) && json.data.length > 0) {
            ...
            const dbIds = new Set(apiTrips.map(t => String(t.dbTripId)))
            const merged = [...apiTrips, ...localTrips.filter(t => !t.dbTripId || !dbIds.has(String(t.dbTripId)))]
            applyTrips(merged)
          }
        })
        .catch(() => {})
    ...
    if (useBackend || (token && activeTripId !== 'all' && activeTripId !== 'standalone')) {
      try {
        ...
        const body = {
          ...
          expense_type: 'ACTUAL',
          trip_id: activeTripId !== 'all' ? activeTripId : undefined,
        ...
        if (res.ok && data.success) {
          ...
          const lTripId    = String(activeTripId)
  ...
  const effectiveBudget = budget ? Math.max(budget - hotelBudget, 0) : 0

  /* ── display-currency helpers (defined early so all derived vars can use them) ── */
  const currSym = symFor(displayCurrency)
  // eslint-disable-next-line no-shadow
  const fmt  = (val) => `${currSym} ${Number(val || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
  const fmtB = fmt
  ...
  const alerts = useMemo(() => {
    ...
    return a
  }, [budget, budgetDisplay, budgetPct, totalSpent])
