...
function Profile({ theme, toggleTheme }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/users/profile`, {
      ...
      .then(res => {
        ...
        // Refresh preferences & privacy states from DB
  ...
  const [preferences, setPreferences] = useState({
    ...
    smsNotifications: storedPrefs.smsNotifications ?? false,
    promotionalEmails: storedPrefs.promotionalEmails ?? true,
    newsletterSubscription: storedPrefs.newsletterSubscription ?? true,
    ...
    if (token) {
      try {
        ...
        if (!res.ok) {
          ...
          showSaved(`Warning: ${d.message || 'Server error, local changes saved.'}`)
          setIsEditing(false)
          return
        }
  ...
  const handlePreferencesUpdate = async (e) => {
    ...
    // Sync currency to the shared key used by Dashboard, ExpenseTracker, HotelPicker, TripBudget
      ...
      try {
        await fetch(`${API_BASE}/users/profile`, {
          ...
          body: JSON.stringify({
            address: {
              ...(existing.address || {}),
              prefs: { currency: preferences.currency, preferred_weather: preferences.preferred_weather },
            },
          }),
        })
    ...
    if (token) {
      try {
        await fetch(`${API_BASE}/users/profile`, {
          ...
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
          body: JSON.stringify({
            address: { ...(existing.address || {}), privacy },
          }),
        })
      } catch { /* save locally regardless */ }
    }
  ...
  const saveAvatar = async (src) => {
    ...
    if (token) {
      try {
        ...
        if (!res.ok) {
          showSaved('Photo saved locally (sync failed, will retry on next save)')
          setShowAvatarModal(false)
          return
        }
      } catch {
        showSaved('Photo saved locally (offline)')
        setShowAvatarModal(false)
        return
      }
    }
    setShowAvatarModal(false)
    showSaved(src ? 'Profile photo updated!' : 'Profile photo removed!')
  }
  ...
  const handleFilePick = (e) => {
    ...
    reader.onerror = () => showSaved('Failed to read image file. Please try another.')
