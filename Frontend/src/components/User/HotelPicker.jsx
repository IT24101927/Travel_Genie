...
/* ─── Static fallback data removed - hotels now served from the DB ─── */
const _FALLBACK_HOTELS_REMOVED = {
  sigiriya: [
    {
...
/* ─── Per-district static data removed - no longer used ─── */
const _HOTELS_BY_DISTRICT_REMOVED = {
  // Colombo (d1)
  d1: [
...
// Empty replacements so references below still work
const FALLBACK_HOTELS = { default: [] }
...
const mapHotel = (h) => ({
  ...
  priceRange:  { min: parseFloat(h.price_per_night) || 0, max: null, currency: 'USD' },
  ...
  images:      (h.place?.images || []).map(img => ({ url: img.image_url || img.url || '' })),
  rating:      h.rating       || 0,
  hotel_id:    h.hotel_id,
  place_id:    h.place_id,
})
