...
function PlanTrip({ theme, toggleTheme }) {
  ...
  // Map of district name → DB district_id (populated from API)
  const [dbDistrictIds, setDbDistrictIds] = useState({})

  // Fetch DB district IDs on mount to link static data with DB
  useEffect(() => {
    fetch(`${API}/districts`)
      .then(r => r.json())
      .then(json => {
        if (json.success && Array.isArray(json.data)) {
          const map = {}
          json.data.forEach(d => { map[d.name] = d.district_id })
          setDbDistrictIds(map)
        }
      })
      .catch(() => {/* offline — district trips will save locally only */})
  }, [])
  ...
  const handleSelect = (district) => {
    // Attach the DB district_id (if available) so TripDetails can send it to the API
    const enriched = { ...district, district_id: dbDistrictIds[district.name] || null }
    localStorage.setItem('selectedDistrict', JSON.stringify(enriched))
