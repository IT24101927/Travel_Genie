...
/* ──── District name → legacy string-ID map (used by DistrictExplore / PLACES_BY_DISTRICT) ──── */
...
function DistrictCard({ district, onSelect }) {
  ...
  // Reset error flag whenever the image src changes (e.g. after DB data loads)
  useEffect(() => { setImgError(false) }, [district.image])
      ...
      .then(json => {
        if (json.success && Array.isArray(json.data)) {
          setDistricts(json.data.map(d => ({
            id:          DISTRICT_NAME_TO_ID[d.name] || null,
